...
from api import (add_to_cart, clear_user_cart, create_order, get_display_cart,
                 get_image, get_or_create_user_cart, get_products,
                 get_user_cart_with_items, init_strapi_session,
                 remove_from_cart)
...
def handle_menu(update, context):
    ...
    count_items = get_display_cart(cart_items).get("count_items")
...
def handle_quantity_selection(update, context):
    ...
    query_quantity.answer(text=f"Выбрано: {quantity}", show_alert=False)
...
def handle_remove_product(update, context):
    ...
    query.answer(text="Товар удален из корзины!", show_alert=False)
